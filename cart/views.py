...
def add_cart(request, product_id):
    ...
    try:
        ...
        cart_total = {
            'subtotal': str(total),
            # 'tax': str(round((Decimal('0.02') * total), 2)),  # 2% tax - commented out
            'grand_total': str(total)  # Removed tax calculation
        }
...
def update_cart(request, product_id, cart_item_id):
    ...
    try:
        ...
        # tax = round((Decimal('0.02') * cart_total), 2)  # 2% tax - commented out
        grand_total = cart_total  # Removed tax calculation
        
        # Calculate sub_total for the cart_item for display in the template
        cart_item.sub_total = item_subtotal
        
        return JsonResponse({
            'success': True,
            'item_subtotal': str(item_subtotal),
            'cart_total': str(cart_total),
            # 'tax': str(tax),  # Commented out tax
...
def remove_cart(request, product_id, cart_item_id):
    ...
    try:
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            ...
            total_price = sum([
                (item.product.price + (25 if any(var.variation_category.lower() == 'marination' for var in item.variation.all()) else 0)) * item.quantity 
                for item in cart_items
            ])
            # tax = round(((2 * total_price)/100), 2)  # Commented out tax calculation
            grand_total = total_price

            # Calculate item subtotal if it still exists
            if cart_item.pk:  # Check if the item still exists in the database
                item_variation_price = Decimal('25.00') if any(
                    var.variation_category.lower() == 'marination' 
                    for var in cart_item.variation.all()
                ) else Decimal('0.00')
                item_subtotal = (cart_item.product.price + item_variation_price) * cart_item.quantity
            else:
                item_subtotal = 0
    ...
    try:
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            ...
            total_price = sum([
                (item.product.price + (25 if any(var.variation_category.lower() == 'marination' for var in item.variation.all()) else 0)) * item.quantity 
                for item in cart_items
            ]) if cart_items.exists() else Decimal('0.00')
...
def cart(request, total_price=0, quantity=0, cart_items=None):
    try:
        ...
        # Calculate total with variation price and add subtotal to each item
        total_price = 0
        for item in cart_items:
            variation_price = 25 if any(var.variation_category == 'marination' for var in item.variation.all()) else 0
            item.sub_total = (item.product.price + variation_price) * item.quantity
